...
class BPSMouseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            # Apply the TransformsSimCLR to the image
            img_tensor1, img_tensor2 = self.transform(img_array)
            return img_tensor1, img_tensor2, \
                particle_type_tensor, particle_type_tensor,\
                img_key

        else:
            return nn.ToTensor()(img_array), nn.ToTensor()(img_array), \
                particle_type_tensor, particle_type_tensor, \
                img_key
...
def main():
    ...
    training_bps = BPSMouseDataset(train_csv_file,
                                   local_train_dir,
                                   transform=TransformsSimCLR(resize=300, size=254),
                                   file_on_prem=True)
